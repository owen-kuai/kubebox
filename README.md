...
- `internal/operator`：controller-runtime `SandboxClaim` Reconcile、Kubernetes Pod adapter（注入 sandbox id、envd 双端口与 emptyDir 沙箱卷）、finalizer/status 回填
- `internal/dataplane`：envd-proxy 路由、短期 scope JWT、凭证剥离转发、注入 sandbox id 与派生 scope 供门面复核
...
数据面已形成完整可执行链路：`envd-proxy`（公网边界校验短期 scope JWT、剥离凭证）→ 注入 sandbox id 与派生 scope → `envd` HTTP 门面（可信容器内复核身份/scope）→ `ProcessExecutor`（沙箱 root 内真实命令与文件 IO）。`cmd/envd` 以主进程嵌入沙箱容器（镜像 `deploy/docker/envd.Dockerfile`），`KubePodClient` 创建 Pod 时注入 sandbox id、挂载 emptyDir `/sandbox` 并暴露 gRPC/HTTP 双端口。
